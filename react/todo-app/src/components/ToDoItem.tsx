...
const ToDoItem = ({item, onDelete} : Props) => {
    ...
    return (
    <div className='w-[90%] md:w-4/6 border p-3 flex rounded-md bg-slate-400 text-white md:text-base text-sm'>
        ...
        <div className='w-2/6 flex gap-2 justify-end items-center'>
            <input className='sm:h-8 sm:w-8' type="checkbox" onClick={() => setChecked(!checked)}/>
            <button className='w-4/6 bg-red-600 rounded-md text-xs py-3' onClick={() => onDelete(item.id)}>Delete</button>
        </div>
    </div>
  )
}
